# Website hero — PRO version: the BTS S-curve at Chong Nonsi

- **Usage**: background video for the Nivas Companion homepage. It dresses the page
  behind the title and CTA — it must never distract.
- **Why this shot**: the BTS S-curve at Chong Nonsi with the MahaNakhon tower is THE
  shot every Bangkok agent recognizes in 0.5 s. It's their city seen from their daily
  life (the Skytrain), not the tourist postcard.
- **Model**: Seedance 2.0
- **Format**: 16:9 · 8 s · fixed shot (no camera movement, no cuts) · loop
- **Post-production**: muted (browser autoplay), export compressed mp4/webm as a loop.
  No text to burn in — the site title lives on top.
- **Loop tip**: generate 3–4 takes; if the loop seam is visible, redo on Kling with
  *first frame = last frame* (same empty-track image at both ends).
